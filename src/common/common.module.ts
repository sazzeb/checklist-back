    ...
    imports: [
        ...
        BullModule.forRootAsync({
            ...
            useFactory: (configService: ConfigService) => ({
                connection: {
                    host: !isDocker
                        ? configService.get<string>('REDIS_HOST') // Use Docker host
                        : configService.get<string>('REDIS_HOST_DOCKER'), // Use local host,
...
export class CommonModule implements OnModuleInit {
    ...
    async onModuleInit() {
        const isRedisAvailable = await this.redisProvider.isRedisAvailable();
        if (!isRedisAvailable) {
            this.logger.error('Redis is not available. Aborting startup.');
            process.exit(1); // Exit the application if Redis is critical
        }

        this.logger.log('Redis is ready. Loading other components...');
    }
}
